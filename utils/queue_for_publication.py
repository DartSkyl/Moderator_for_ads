...
async def publisher_function(ads: ContainerForAds, shed=False):
    ...
    if ads.file_id:  # Если данный список пуст, значит объявление без медиафайлов
        media_group = MediaGroupBuilder(caption=(html.quote(ads.text) + msg_with_validity))
        for mediafile in ads.file_id:
            media_group.add(type=mediafile[1], media=mediafile[0])
        message = await bot.send_media_group(chat_id=MAIN_GROUP_ID, media=media_group.build())
...
class Publisher:
    ...
    async def auto_remove_ads(self, message_id: int, validity):
        """Функция автоматически удаляет объявления, которые действительны не более чем 2 суток"""
        # Так как бот может удалять только сообщения не старше чем 2 суток, то что б наверняка, отнимем 10 минут
        validity = (validity * 24 * 60) - 10
